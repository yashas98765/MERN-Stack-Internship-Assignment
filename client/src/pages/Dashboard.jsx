  ...
  return (
    <div className={`task ${t.status}`}>
      <div className="row">
        ...
        <div>
          <span className={t.status === 'completed' ? 'completed-indicator' : 'meta'}>{t.status}</span>
        </div>
      </div>
      {t.description && <p style={{marginTop:8}}>{t.description}</p>}
      <div className="actions">
        <button className="btn-icon" onClick={() => onToggle(t._id)}>{t.status === 'pending' ? 'Complete' : 'Uncomplete'}</button>
        <button className="btn-icon" onClick={() => onEdit(t)}>Edit</button>
        <button className="btn-icon" onClick={() => onDelete(t._id)}>Delete</button>
      </div>
    </div>
  )
}
    ...
    <div>
      <div className="dashboard-head">
        ...
        <div className="controls">
          <input className="search" placeholder="Search tasks..." value={q} onChange={(e)=>{setQ(e.target.value); setPage(1)}} />
      ...
      <div className="card">
        ...
        <div className="pagination">
          ...
          {[...Array(pages)].map((_,i)=>(
            <button key={i} className={`page-btn ${i+1===page? 'active': ''}`} onClick={()=>setPage(i+1)}>{i+1}</button>
          ))}
